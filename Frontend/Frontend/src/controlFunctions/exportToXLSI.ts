...
export const exportToExcel = async (
  ...
  tableData.forEach((row) => {
    ...
    newRow.getCell(3).value = {
      text: encapsulatedLink,
      hyperlink: encapsulatedLink,
    };
...
export const exportToExcelPage = async (tableData: PageurlType[]) => {
  const workbook = new ExcelJS.Workbook();
...
export const exportToExcelMyUrls = async (tableData: urlType[]) => {
  ...
  tableData.forEach((row) => {
    ...
    newRow.getCell(2).value = {
      text: encapsulatedLink as string,
      hyperlink: encapsulatedLink as string,
    };
    newRow.getCell(2).font = { color: { argb: "0563C1" }, underline: true };
  });
